        ...
        if (multiple) {
            const newValue =
                value?.includes(selectedValue) && Array.isArray(value)
                    ? (value as PathValue<TFieldValues, Path<TFieldValues>>[]).filter(v => v !== selectedValue)
                    : [...(value ?? []), selectedValue];
            onChange?.(newValue);
        } else {
                            ...
                            >
                                ...
                                >
                                    {(value && value.length > 0) || typeof value === "number" ? (
                                        multiple ? (
                                            (options || [])
                                                .filter(option => Array.isArray(value) && value.includes(option.id))
                                                .map(option => (
                                                    <span
                                                        key={option.id}
                                                ...
                                                {(options || []).map(option => {
                                                    ...
                                                    return (
                                                        <CommandItem
                                                            key={option.id}
                                                            // value={option.value}
                                                            onSelect={() => handleSelect(option.id, onChange, value)}
                                                        >
                                                            {multiple && (
                                                                <div
                                                                    className={cn(
