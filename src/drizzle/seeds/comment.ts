...
import { CommentSchema } from "@/drizzle/schema/comment";
import { faker } from "@faker-js/faker";

const parentCommentsMock = async () => {
    const [postsData, usersData] = await Promise.all([db.query.post.findMany(), db.query.user.findMany()]);
...
const childCommentsMock = async () => {
    const [commentsData, usersData] = await Promise.all([db.query.comment.findMany(), db.query.user.findMany()]);
