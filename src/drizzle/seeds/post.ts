...
import { PostSchema } from "@/drizzle/schema/post";
import { faker } from "@faker-js/faker";

const mock = async () => {
    const [usersData, categoriesData] = await Promise.all([db.query.user.findMany(), db.query.category.findMany()]);

    const data: Omit<Extract<PostSchema, { mode: "create" }>, "tagIds" | "mode">[] = [];
