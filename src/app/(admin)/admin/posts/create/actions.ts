...
import { post, postTags } from "@/drizzle/schema";
import { PostSchema, postSchema } from "@/drizzle/schema/post";
...
export async function createPost(data: PostSchema) {
    return executeAction({
        actionFn: async () => {
            ...
            const { postId } = (await db.insert(post).values(validatedData).returning({ postId: post.id }))[0];

            if (validatedData.tagIds.length > 0) {
                await db.insert(postTags).values(validatedData.tagIds.map(tagId => ({ postId, tagId })));
            }
    ...
    return executeAction({
        actionFn: async () => {
            ...
            if (validatedData.mode === "edit") {
                await db.update(post).set(validatedData).where(eq(post.id, +validatedData.id));
