...
// }),
export async function getPosts(page: number, limit: number, searchTerm?: string) {
    return executeQuery({
        queryFn: async () =>
            db
                .select()
...
export async function getUserPosts({ limit, page, userId }: { limit: number; page: number; userId: number }) {
    return executeQuery({
        queryFn: async () =>
            await db.query.post.findMany({
                where: eq(post.userId, userId),
